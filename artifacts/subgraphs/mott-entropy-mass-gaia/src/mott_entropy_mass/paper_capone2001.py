...
capone_fl_entropy_order_unity_at_tk = claim(
    r"""For the Capone-Fabrizio-Tosatti Fermi-liquid side, S(T)/N ~ T/T_K for T <= T_K, so the entropy per site at T = T_K remains finite and order unity for every nonzero T_K even as T_K tends toward zero [@Capone2001].""",
    lkm_id="gcn_31bc66ca16a44508",
    source_paper="paper:867770551072981407",
    provenance_source="lkm",
    decomposition_of="gcn_31bc66ca16a44508",
    lkm_original=_FL_ENTROPY_ORIGINAL,
)
...
capone_entropy_mismatch_requires_instability = claim(
    r"""Under the Capone-Fabrizio-Tosatti assumptions of fixed chemical potential, no spontaneous symmetry breaking, and a nondegenerate singlet Mott endpoint, the order-unity Fermi-liquid entropy at T ~ T_K cannot disappear continuously as T_K -> 0; the entropy mismatch requires a thermodynamic instability or distinct intermediate phase [@Capone2001].""",
    lkm_id="gcn_dd12256615264dfb",
    source_paper="paper:867770551072981407",
    provenance_source="lkm",
    decomposition_of="gcn_dd12256615264dfb",
    lkm_original=_CONTINUITY_ORIGINAL,
)
...
strat_decompose_entropy_premise = deduction(
    [gcn_31bc66ca16a44508],
    capone_fl_entropy_order_unity_at_tk,
    reason=(
        "1. Premise gcn_31bc66ca16a44508 explicitly gives S(T)/N ~ T/T_K for "
        "T <= T_K and therefore S(T_K) is finite and order unity for T_K > 0."
    ),
    prior=0.95,
)
...
strat_decompose_continuity_premise = deduction(
    [gcn_dd12256615264dfb],
    capone_entropy_mismatch_requires_instability,
    reason=(
        "1. Premise gcn_dd12256615264dfb explicitly states that absent "
        "chemical-potential motion or symmetry breaking, a thermodynamic "
        "instability or new intermediate phase must remove the entropy."
    ),
    prior=0.95,
)
